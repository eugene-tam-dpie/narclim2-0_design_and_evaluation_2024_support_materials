...
DPIE fork of https://github.com/coecms/WRF.git

WRF and WPS themselves should be the same as CCRC, with the only difference being the build scripts

## Instructions to clone

Goto intended run directory (e.g. `cd /scratch/<project>/<username>`)

This directory referred to `$runpath` from here onwards

Run `git clone git@bitbucket.org:oehcas/wrf_narclim.git`

## Instructions to compile WRF

Run `cd $runpath/wrf_narclim/WRFV3/`

Run `./run_compile`

WRF executable should be available from `$runpath/WRFV3/run`

## Instructions to compile WRF

Run `cd $runpath/wrf_narclim/WPS/`

Run `./run_compile`

## Known issues

* You must check your `$PROJECT` value is the one you intend to charge SU's for compilation
    * Failure to do so may lead to compilation jobs not starting.

# Original CCRC README 
